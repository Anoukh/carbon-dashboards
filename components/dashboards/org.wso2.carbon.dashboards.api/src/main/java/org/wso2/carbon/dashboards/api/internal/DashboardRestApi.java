...
public class DashboardRestApi implements Microservice {
    ...
    public Response delete(@PathParam("id") String id, @Context Request request) {
        try {
            ...
            LOGGER.error(String.format("An error occurred when deleting dashboard %s", replaceCRLFCharacters(id)), e);
    ...
    public static String replaceCRLFCharacters(String str) {
        return str.replace('\n', '_').replace('\r', '_');
    }
